...
public interface MessageRepository extends JpaRepository<MessageFile, Integer> {
	...
	@Query(value="SELECT e.message_id,e.start_line,e.requirementid,e.end_line,e.message,e.description,e.short_description,e.query,f.file_path from message_file as e " + 
	...
	@Transactional
	@Modifying
	@Query(value="INSERT INTO message_file(description,end_line,message,query,start_line,short_description,file_id,requirementid) "
			+ "VALUES (:desc,:end_line,:msg,:quer,:start,:short_desc,:file,:requirement)",nativeQuery=true)
